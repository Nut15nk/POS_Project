  ...
  const ProductCard = ({ product }) => {
    return (
      <div className="product-card" style={{ overflow: 'hidden', maxHeight: '100%' }}>
        ...
        <p className={`stock ${product.stock < 2 ? 'low' : 'normal'}`}>
        ...
        {product.createdBy?.address?.province ? (
          <p className="province">จังหวัด: {product.createdBy.address.province}</p>
        ) : (
          <p className="province">จังหวัด: ไม่ระบุ</p>
        )}
      </div>
    );
  };
